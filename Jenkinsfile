...
pipeline {
    ...
    stages {
        stage('Deploy on k8s') {
            steps {
                withCredentials([ string(credentialsId: 'minikube-credentials', variable: 'secret') ]) {
                    sh 'kubectl --secret ${secret} --server https://host.docker.internal:${env.K_PORT}  --insecure-skip-tls-verify=true apply -f ./k8s/configmap.yaml'
